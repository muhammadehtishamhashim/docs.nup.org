...
export const metadata = {
  ...
  description: 'BS Computer Science Semester 4 Documentation',
}

const banner = <Banner storageKey="nup-bs-cs-sem-4">BS Computer Science Semester 4 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Computer Science - Semester 4</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-cs/sem-4')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-cs/sem-4" } }} />}
    >
      <div data-pagefind-filter="major: bs-cs/sem-4">
        {children}
      </div>
    </Layout>
  )
