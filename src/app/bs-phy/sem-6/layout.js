...
export const metadata = {
  title: 'BS Physics - Semester 6',
  description: 'BS Physics Semester 6 Documentation',
}

const banner = <Banner storageKey="nup-bs-phy-sem-6">BS Physics Semester 6 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Physics - Semester 6</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-phy/sem-6')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-phy/sem-6" } }} />}
    >
      <div data-pagefind-filter="major: bs-phy/sem-6">
        {children}
      </div>
    </Layout>
  )
