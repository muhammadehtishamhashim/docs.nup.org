...
export const metadata = {
  title: 'BS Physics - Semester 1',
  description: 'BS Physics Semester 1 Documentation',
}

const banner = <Banner storageKey="nup-bs-phy-sem-1">BS Physics Semester 1 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Physics - Semester 1</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-phy/sem-1')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-phy/sem-1" } }} />}
    >
      <div data-pagefind-filter="major: bs-phy/sem-1">
        {children}
      </div>
    </Layout>
  )
