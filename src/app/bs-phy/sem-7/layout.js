...
export const metadata = {
  title: 'BS Physics - Semester 7',
  description: 'BS Physics Semester 7 Documentation',
}

const banner = <Banner storageKey="nup-bs-phy-sem-7">BS Physics Semester 7 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Physics - Semester 7</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-phy/sem-7')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-phy/sem-7" } }} />}
    >
      <div data-pagefind-filter="major: bs-phy/sem-7">
        {children}
      </div>
    </Layout>
  )
