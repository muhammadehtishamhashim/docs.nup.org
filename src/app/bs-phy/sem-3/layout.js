...
export const metadata = {
  title: 'BS Physics - Semester 3',
  description: 'BS Physics Semester 3 Documentation',
}

const banner = <Banner storageKey="nup-bs-phy-sem-3">BS Physics Semester 3 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Physics - Semester 3</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-phy/sem-3')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-phy/sem-3" } }} />}
    >
      <div data-pagefind-filter="major: bs-phy/sem-3">
        {children}
      </div>
    </Layout>
  )
