...
export const metadata = {
  title: 'BS Physics - Semester 4',
  description: 'BS Physics Semester 4 Documentation',
}

const banner = <Banner storageKey="nup-bs-phy-sem-4">BS Physics Semester 4 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Physics - Semester 4</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-phy/sem-4')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-phy/sem-4" } }} />}
    >
      <div data-pagefind-filter="major: bs-phy/sem-4">
        {children}
      </div>
    </Layout>
  )
