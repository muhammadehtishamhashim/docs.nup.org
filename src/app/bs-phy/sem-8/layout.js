...
export const metadata = {
  title: 'BS Physics - Semester 8',
  description: 'BS Physics Semester 8 Documentation',
}

const banner = <Banner storageKey="nup-bs-phy-sem-8">BS Physics Semester 8 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Physics - Semester 8</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-phy/sem-8')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-phy/sem-8" } }} />}
    >
      <div data-pagefind-filter="major: bs-phy/sem-8">
        {children}
      </div>
    </Layout>
  )
