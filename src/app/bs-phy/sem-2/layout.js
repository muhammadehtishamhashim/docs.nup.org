...
export const metadata = {
  title: 'BS Physics - Semester 2',
  description: 'BS Physics Semester 2 Documentation',
}

const banner = <Banner storageKey="nup-bs-phy-sem-2">BS Physics Semester 2 Documentation 🎓</Banner>
const navbar = (
  <Navbar
    logo={<b>NUP - BS Physics - Semester 2</b>}
  >
...
export default async function SemesterLayout({ children }) {
  ...
  const semesterItem = majorItem?.children?.find(item => item.route === '/bs-phy/sem-2')
  ...
  return (
    <Layout
      ...
      footer={footer}
      search={<Search searchOptions={{ filters: { major: "bs-phy/sem-2" } }} />}
    >
      <div data-pagefind-filter="major: bs-phy/sem-2">
        {children}
      </div>
    </Layout>
  )
